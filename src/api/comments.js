...
export function sendComment(marketId, commentId, marketType) {
  return getMarketClient(marketId)
    .then((client) => client.investibles.updateComment(commentId, undefined, undefined,
      undefined, undefined, undefined, undefined, true, undefined, undefined, undefined, marketType))
    .catch((error) => toastErrorAndThrow(error, 'errorCommentSaveFailed'));
}

export function alterComment(marketId, commentId, notificationType, groupId) {
  return getMarketClient(marketId)
    .then((client) => client.investibles.alterComment(commentId, notificationType, groupId))
    .catch((error) => toastErrorAndThrow(error, 'errorCommentSaveFailed'));
}
...
export function updateComment(values) {
  const { marketId, commentId, body, commentType, uploadedFiles, mentions, notificationType,
    isSent, allowMulti, isRestricted, inProgress, isVisible, version, tz, resolved, creationStageId } = values;
  return getMarketClient(marketId)
    .then((client) => client.investibles.updateComment(commentId, body, resolved, uploadedFiles, mentions,
      ...
      tz ?? (body || commentType ? getBrowserTz() : undefined), creationStageId))
    .catch((error) => toastErrorAndThrow(error, 'errorCommentSaveFailed'));
}
